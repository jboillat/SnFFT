...
#	- YSRnp[i] is Young's Seminormal Representation for the adjacent transposition (i, i + 1) corresponding to the pth partition of n
# Return Values:
# 	RM::Array{BigInt, 2}
#	- Young's Seminormal Representation of P corresponding to the pth partition of n
function ysr_permutation(M::Int, P::Array{Int, 1}, YSRnp::Array{SparseMatrixCSC, 1})
	...
	if length(ATF) == 0
		...
		return RM
                ...
                #@assert typeof(YSRnp[ATF[1]]) == Array{BigInt, 1}
		RM = copy(full(YSRnp[ATF[1]]))
		for i = 2:length(ATF)
			RM = (RM * full(YSRnp[ATF[i]])) % M
		end
		return RM
	end
end
