...
type IntegerMod{P} <: Number
    ...
    function IntegerMod(N::BigInt)
        new(mod(N,P), P)
    end
end

function makeModular(n::BigInt, p::Integer)
    return IntegerMod{p}(n)
end

convert{P}(::Type{IntegerMod{P}}, n::BigInt) = IntegerMod{P}(n)
convert{P}(::Type{IntegerMod{P}}, a::Array) = [IntegerMod{P}(n) for n in a]

promote_rule{P}(::Type{IntegerMod{P}}, ::Type{BigInt}) = IntegerMod{P}
...
function =={P}(a::IntegerMod{P}, b::BigInt)
  return a.n == mod(b, P)
end

function =={P}(a::BigInt, b::IntegerMod{P})
  return mod(a, P) == b.n
end
